...
class B2BEventScraper:
    ...
    def scrape_event2(self):
        try:
            ...
            event_dates_elem = soup.find_all("h3", class_='ck-strapline--colour-white')
            re_pattern = re.compile(r'(\b\w+ \d+(?:th|rd|nd|st)?(?: & \d+(?:th|rd|nd|st)?)?,\s*\d{4})')
            for elem in event_dates_elem:
                text = elem.get_text().strip()
                if not event_details["Event Date(s)"]:
                    match = re_pattern.search(text)
                    if match:
                        event_details["Event Date(s)"] = match.group(1)
                if not event_details["Location"]:
                    location_parts = text.split(' - ')
                    if len(location_parts) > 0:
                        event_details["Location"] = location_parts[0]
